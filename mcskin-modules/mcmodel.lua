...
local function qs_Draw(A,gc,AA,showWireframe,matProj,depth)
    ...
    if n == 1 then
        ...
        face:drawPixels(gc,AA,showWireframe,matProj, gcw, gch)
    ...
    local best_i = 0
    local best_hit = 99999
    local best_hit_list = {}
    
    local test_count = 3 --this number was tested to give the best results

    local i = 1
    local m = min(test_count, n)

    while i <= m and best_hit > 0 do
        local i_rnd = random(n)
    ...
    s:drawPixels(gc,AA,showWireframe,matProj, gcw, gch)
...
function MCModel:draw(camera, gc, light_dir, AA,showWireframe)
    ...
    for key, part in pairs(self) do
        ...
        local worldView = Mat4x4.matMul(Mat4x4.matMul(worldRot, worldPos),matView)
        ...
        for _, cube in pairs(part.mesh) do
            ...
            for i=1, 6 do
                ...
                local pointBuffer = {}
                for j=1, 4 do
                    pointBuffer[j] = face[j]:applyMat4x4(worldView)
                end

                local line1 = pointBuffer[2] - pointBuffer[1]
                local line2 = pointBuffer[4] - pointBuffer[1]

                local normal = Vec3.norm(Vec3.cross(line1,line2))
                local isFacingCamera = Vec3.dot(normal, pointBuffer[1]) < 0 -- normal.z < 0 --
                if (not cube.isBackfaceCulling) or isFacingCamera then
                    ...
                    projTri.p[1] = pointBuffer[1]
                    projTri.p[2] = pointBuffer[2]
                    projTri.p[3] = pointBuffer[3]
                    projTri.p[4] = pointBuffer[4]
                    projTri.t[1] = uv[1]
                    projTri.t[2] = uv[2]
                    projTri.t[3] = uv[3]
                    projTri.t[4] = uv[4]
    ...
    --painters algo sorting references https://education.siggraph.org/static/HyperGraph/scanline/visibility/painter.htm
    ...
    randomseed(camera.rot.z,camera.rot.y) --stable partitioning when viewing from the same angle.
